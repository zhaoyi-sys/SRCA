...
def predicted_class_thresholds(
    train_score: np.ndarray,
    train_pred: np.ndarray,
    percentile: float,
    num_classes: int,
) -> np.ndarray:
    global_threshold = float(np.percentile(train_score, float(percentile)))
    thresholds = np.full(int(num_classes), global_threshold, dtype=np.float32)
    for class_idx in range(int(num_classes)):
        mask = train_pred.astype(np.int64) == class_idx
        if np.any(mask):
...
def global_thresholds(
    ...
    threshold = float(np.percentile(train_score, float(percentile)))
    return np.full(int(num_classes), threshold, dtype=np.float32)
